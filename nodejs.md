...
安装好node之后，会默认安装好np。npm本身也是Node.js开发的软件
...
^3.6.0表示安装包的时候会将后边两位装成最新的，相当于^3.x.x，如果最新的是3.7.3，那么装的 就是3.6.9

~3.6.0表示安装包的时候会将后边两位装成最新的，相当于~3.6.x，如果罪行的是3.6.5，那么装的就是3.6.5
